...
//populate array 
gameData.forEach(function(ele) {
  Stage.data.push(new Stage(ele));
})
...
Stage.data.forEach(function(stages) {
  $('#stageDataTemplate').append(stages.toHtml());
});
...
function loadGame(){
   player = JSON.parse(localStorage.LOTR);
